...
limit. Curved coordinates and anisotropic coefficients are not supported.
...
A spatially varying KINEMATIC coefficient is available through the Python API
only: set ``nu_scale_x1`` / ``kappa_scale_x1`` on ``DiffusionOptions`` before
the mesh block is built, one entry per x1 cell centre including ghost cells.
There is no YAML form, and a profile set after the block is built is refused.
