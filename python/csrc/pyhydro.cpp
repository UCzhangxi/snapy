...
void bind_hydro(py::module& m) {
  ...
  m.def("balance_column", &snap::balance_column, py::arg("w"),
        py::arg("dx1f"), py::arg("grav"), py::arg("wall_clamp") = true,
        py::arg("rtol") = 1.e-10, py::arg("max_iter") = 120,
...
does not converge. See src/hydro/balance_column.hpp for the caller's two
obligations (physical x1 boundaries; >= 5 x1 cells per block).)");
}
