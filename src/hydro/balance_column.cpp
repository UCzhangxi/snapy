...
std::tuple<torch::Tensor, double, int> balance_column(torch::Tensor const& w,
  ...
  TORCH_CHECK(dx1f.scalar_type() == w.scalar_type() &&
                  dx1f.device() == w.device(),
  ...
  for (; sweeps < max_iter; ++sweeps) {
    ...
    TORCH_CHECK(prs.min().item<double>() > 0.,
                "balance_column: the projection drove the pressure non-positive "
                "on sweep ", sweeps, " -- the column is diverging, not converging.");
  }
