...
bool MeshBlockImpl::limiter_patch_hit() const {
  ...
  return m.defined() && m[0].item<bool>();
}

bool MeshBlockImpl::limiter_nan_hit() const {
  auto const &m = phydro->peos->limiter_marks();
  return m.defined() && m[1].item<bool>();
}
...
int MeshBlockImpl::check_redo(Variables &vars) {
  ...
  auto flag = torch::tensor(
      {floor_hit(vars) ? 1. : 0., vic_dry_clamp_hit() ? 1. : 0.,
       limiter_patch_hit() ? 1. : 0., limiter_nan_hit() ? 1. : 0.},
