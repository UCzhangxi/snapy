...
int MeshBlockImpl::check_redo(Variables &vars) {
  ...
  // so the old check fired a fatal (dt-halving-proof) redo on a valid cold layer
  // [ISSUES C5]. Screen cheaply on the conserved energy (for a 0 K-referenced
  // EOS E <= 0  =>  P <= 0 -- one direction only, but it is the direction we
  // need: the screen never misses a genuine negative pressure) and confirm with
  // a cons2prim pressure check only when it trips. Behavior-identical to the old
  // check on any run that never trips the screen (all current ideal-gas/moist
  // production). NOTE: under eos.limiter=true cons2prim floors P to
  // pressure_floor(>0), so this pressure-redo is a no-op there and pathological
  // states are absorbed by the limiter floor instead; a valid cold state has
  // physical P>0 far above the floor, so the C5 fix still holds.
