...
constexpr double kGrav = 10.44;  // Jupiter-like, as the S115 probe column
...
Column marched_column(int nx1, double height, bool uniform) {
  ...
  auto rho_t = w[snap::IDN], prs_t = w[snap::IPR];  // accessor() is deleted on an rvalue
  ...
  for (int i = 0; i < nx1; ++i) {
    ...
    // forward Euler from the previous centre: the truncation S115 removes
    if (i > 0) {
...
TEST(BalanceColumn, a_ghost_free_column_reproduces_a_blocks_own_reference) {
  for (bool uniform : {true, false}) {
    ...
    auto blk_ref = reference(b.w, b.dx1f, 3 + nx1 - 1, uniform, true, true, true);

    EXPECT_TRUE(torch::equal(free_ref.pref, blk_ref.pref.narrow(-1, 3, nx1)))
        << "uniform=" << uniform;
    EXPECT_TRUE(torch::equal(free_ref.psf_lo, blk_ref.psf_lo.narrow(-1, 3, nx1)))
        << "uniform=" << uniform;
    EXPECT_TRUE(torch::equal(free_ref.psf_hi, blk_ref.psf_hi.narrow(-1, 3, nx1)))
        << "uniform=" << uniform;
    ...
    // rather than on i > il, so a ghost-free column takes rop_smooth(il) where a
    // block averages it with rop_smooth(il-1). Both read owned cells only -- the
    // clamp is not leaking -- they weight them differently. dsf plays no part in
    // the fixed point, which reads pref alone, so the balance is unaffected.
    // Asserted rather than omitted, so the exception stays visible if it moves.
...
// ghost-free agree whatever the clamp does. With the clamp OFF they diverge, and
// STRUCTURALLY, not by ghost contamination: the padded wall cell il+1 satisfies
// the interior guard i >= 2 and takes the six-face stencil, while the ghost-free
// cell 1 fails it and keeps the two-point mean. (That ghosts leak at all with
// the clamp off is test_hydro_ref_x1's control, measured there directly.)
...
TEST(BalanceColumn, a_marched_column_comes_out_at_rest) {
  for (bool uniform : {true, false}) {
    ...
    EXPECT_GT(before, 1.e-4) << "the fixture is not the defect: uniform="
                             << uniform;
...
TEST(BalanceColumn, a_block_thinner_than_its_ghosts_keeps_its_columns_apart) {
  ...
  auto make = [&](double scale) {
    ...
    for (int c = 0; c < 2; ++c)
      for (int i = 0; i < nc1; ++i) {
        double s = c == 0 ? scale : 1.0;  // column 1 is the same in both fixtures
...
TEST(BalanceColumn, it_refuses_what_it_cannot_deliver) {
  ...
  EXPECT_THROW(snap::balance_column(c.w.narrow(-1, 0, 4), c.dx1f.narrow(-1, 0, 4),
                                    kGrav),
               c10::Error);
