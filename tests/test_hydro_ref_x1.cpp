...
// ---------------------------------------------------------------------------
// S112 follow-up: the clamp must not cost ACCURACY where it is not needed.
//
...
Column hydrostatic_slab(int n, int ng, int g0, double dz, double H, double p0,
  ...
  auto prs_t = w[snap::IPR], rho_t = w[snap::IDN];  // accessor() is deleted on an rvalue
  auto prs = prs_t.accessor<double, 3>();
  auto rho = rho_t.accessor<double, 3>();
  for (int i = 0; i < nc1; ++i) {
    int g = g0 + (i - ng);
    double p = p0 * std::exp(-(g + 0.5) * dz / H);
    if (g < 0) {  // below the physical wall: the even mirror, deliberately wrong
...
TEST(HydroRefX1Dispatch, the_wall_reference_does_not_depend_on_the_blocks_x1_size) {
  constexpr int ng = 3;
  ...
  // the bottom FOUR of those cells as their own block -- same wall, same column,
  // and the seam anchor the relay would have handed it (hydro.cpp's kWbRefTag)
  ...
  EXPECT_LT(((got - want).abs() / want).max().item<double>(), 1.e-14)
      << "want " << want << "\ngot  " << got;
  EXPECT_TRUE(torch::equal(a.psf_lo.narrow(-1, ng, 4), b.psf_lo.narrow(-1, ng, 4)));
}

// ...and the control for the thin test above it: a four-cell block with walls at
// BOTH ends is the one case where no one-sided row fits, so it really does keep
// the two-point mean -- and with the clamp OFF the ghosts must reach the
